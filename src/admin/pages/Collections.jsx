...
const EMPTY_FORM = { name: '', description: '', image: null, is_featured: false, is_active: true };
...
function CollectionForm({ initial, onSave, onCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (initial) {
        await api.put(`/collections/${initial._id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      } else {
        await api.post('/collections', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      }
      onSave();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-start justify-center p-4 overflow-y-auto" onClick={onCancel}>
      <div className="bg-[#111] border border-white/10 rounded-2xl p-8 w-full max-w-md my-8 shadow-2xl" onClick={e => e.stopPropagation()}>
        <h3 className="text-white font-black text-lg mb-6">{initial ? 'Edit Collection' : 'Add Collection'}</h3>
        <div className="space-y-4">
          <div>
            <label className="text-white/40 text-[10px] font-bold uppercase tracking-widest block mb-1.5">Collection Name</label>
            <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="e.g. Traditional Drop" className="w-full bg-white/5 border border-white/8 text-white rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#B5232B]/50 transition-all" />
          </div>
          <div>
          ...
          <div>
            <label className="text-white/40 text-[10px] font-bold uppercase tracking-widest block mb-1.5">Cover Image</label>
          ...
          <div className="pt-2 flex flex-col gap-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.is_featured} onChange={e => setForm({...form, is_featured: e.target.checked})} className="w-4 h-4 accent-[#B5232B]" />
              <span className="text-white/60 text-sm font-semibold">Featured Collection</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
...
export default function Collections() {
  const { collections, refreshData } = useAdmin();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-white text-2xl font-black tracking-tight">Collections</h1>
          <p className="text-white/40 text-sm mt-0.5">{collections.length} collections configured</p>
        </div>
          ...
          <div key={c._id} className="bg-[#111] border border-white/6 rounded-2xl overflow-hidden hover:border-white/10 transition-all flex flex-col justify-between">
            <div className="aspect-[16/9] bg-[#0d0d0d] relative overflow-hidden flex items-center justify-center">
              ...
              <div className="absolute top-3 left-3 flex gap-1">
                {c.is_featured && (
                  <span className="text-[8px] bg-yellow-500 text-black font-extrabold px-2 py-0.5 rounded tracking-wider uppercase">Featured</span>
                )}
              </div>
